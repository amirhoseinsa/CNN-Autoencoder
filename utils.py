...
def normal_initializer(shape, name=None):
    return tf.Variable(name=name, initial_value=tf.truncated_normal(shape=shape, mean=0, stddev=0.01, dtype=tf.float32))


def zero_initializer(shape, name=None):
    return tf.Variable(name=name, initial_value=tf.zeros(shape=shape, dtype=tf.float32))
